...
sub check_image {
	...
	while (my $line=<$BQUERY>) {
		...
		if ($count != $expect) {
			warn "$ptn_id, $ts, $comp_id: Expecting '$expect', but got '$count'";
		}
		if ($comp_id <= $__comp_id && $ts == $__ts && $ptn_id == $__ptn_id) {
			warn "Unexpected non-increasing component ID: $comp_id, prev_comp_id: $__comp_id";
		}
		$__comp_id = $comp_id;
		$__ptn_id = $ptn_id;
		$__ts = $ts;
	...
	for my $ptn_id (128 .. (128+$NP-1)) {
		if ($sum[$ptn_id] != $cmp_sum[$ptn_id]) {
			warn "ptn_id: $ptn_id, expecting sum: $cmp_sum[$ptn_id], but got $sum[$ptn_id]";
		}
	}
}
