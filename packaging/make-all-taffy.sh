...
if test -f lib/packaging/ovis-lib-toss.spec.in; then
	...
	prefix=`pwd`/.inst
	expected_event2_prefix=$HOME/projects/ldms/Build/libevent-2.0_build
	export CC=gcc
	export CXX=g++
	...
	allconfig="--prefix=$prefix --enable-qc-sampler --enable-rdma --enable-ssl --enable-zaptest --enable-swig --with-ovis-lib=$expected_ovislib_prefix --with-libevent=$expected_event2_prefix --enable-rdma --enable-sos --with-sos=$expected_sos_prefix  --disable-perfevent --disable-rpath" 
