...
export CFLAGS="-O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector --param=ssp-buffer-size=4 -m64 -march=native -fsanitize=address -fno-omit-frame-pointer -pie -fPIC" ; # cflags common to us, libevent2

# local path of scratch ldms files
build_subdir=LDMS_memobj

# full path of where we want things installed
prefix=`pwd`/LDMS_meminst
...
if test -f lib/packaging/ovis-lib-toss.spec.in; then
	...
	if test -f $expected_event2_prefix/include/event2/event.h; then
		...
		if test "$LOCALEVENT" = "1"; then
			if test -f $expected_event2_prefix/include/event2/event.h; then
				...
				mkdir -p .build-eventmem
				cp $eventname.tar.gz .build-eventmem
				(cd .build-eventmem && ../packaging/g49mem.libevent2 $expected_event2_prefix )
				if test -f $expected_event2_prefix/include/event2/event.h; then
	...
	allconfig="--prefix=$prefix --enable-rdma --enable-ssl --with-libevent=$expected_event2_prefix --disable-sos --disable-perfevent --disable-zap --disable-zaptest --disable-swig --enable-auth LDFLAGS=-fsanitize=address"
	../configure $allconfig && \
	make && \
	make install && \
