...
if test -f lib/packaging/ovis-lib-toss.spec.in; then
	...
	authflags="--enable-authentication --enable-authdebug"
	traceflags="--enable-use-tf --enable-use-rtc --enable-use-tid"
	allconfig="--prefix=$prefix --enable-rdma --enable-ssl --with-libevent=$expected_event2_prefix --disable-sos --disable-perfevent --disable-rpath --disable-zap --disable-zaptest --disable-swig $authflags $traceflags"
	../configure $allconfig && \
	make && \
	make install && \
