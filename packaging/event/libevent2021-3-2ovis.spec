Name:           libevent2
Version:        2.0.21
Release:        3.ovis2%{?dist}
Summary:        Abstract asynchronous event notification library

Group:          System Environment/Libraries
License:        BSD
URL:            http://sourceforge.net/projects/levent/
Source0:        http://downloads.sourceforge.net/levent/libevent-%{version}-stable.tar.gz
...
# if Prefix is changed here and in ~/.rpmmacros, update sed exprs below
Prefix: /usr/lib/ovis-ldms
...
%post devel
/sbin/ldconfig
#  this may be a tad bit unportable since lib64 appears
sed -i -e s%/usr/lib/ovis-ldms%$RPM_INSTALL_PREFIX%g $RPM_INSTALL_PREFIX/lib64/pkgconfig/libevent.pc
sed -i -e s%/usr/lib/ovis-ldms%$RPM_INSTALL_PREFIX%g $RPM_INSTALL_PREFIX/lib64/pkgconfig/libevent_openssl.pc
sed -i -e s%/usr/lib/ovis-ldms%$RPM_INSTALL_PREFIX%g $RPM_INSTALL_PREFIX/lib64/pkgconfig/libevent_pthreads.pc
...
%{_libdir}/libevent_openssl.so
%{_libdir}/libevent_pthreads.so
%verify(not md5 size mtime) %{_libdir}/pkgconfig/*.pc
...
%changelog
* Mon Sep 15 2014 Benjamin Allan <baallan@sandia.gov> 2.0.21-stable.ovis2
- TOSS distribution needs libevent2 for ldms fully isolated.
- Change 2.0.21-stable prefix to /usr/lib/ovis-ldms

* Fri Oct 04 2013 Benjamin Allan <baallan@sandia.gov> 2.0.21-stable.ovis1
- Made 2.0.21-stable relocatable and default to /opt/ovis.
- Cannot cohabit in /usr with 1.4 in centos6 or 2.0.21 in same install prefix.
