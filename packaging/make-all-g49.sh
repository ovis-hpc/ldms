...
export LD_LIBRARY_PATH=/home/baallan/gcc/gcc491/lib64:$LD_LIBRARY_PATH

export CFLAGS="-O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector --param=ssp-buffer-size=4 -m64 -march=native -fsanitize=thread -fno-omit-frame-pointer -pie -fPIC" ; # cflags common to us, libevent2
	...
	if test -f $expected_event2_prefix/include/event2/event.h; then
		...
		if test "$LOCALEVENT" = "1"; then
			if test -f $expected_event2_prefix/include/event2/event.h; then
				...
				(cd .build-event && ../packaging/g49.libevent2 $expected_event2_prefix )
				if test -f $expected_event2_prefix/include/event2/event.h; then
