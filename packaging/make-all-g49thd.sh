...
export PATH=/opt/gcc/491/bin:$PATH

export LD_LIBRARY_PATH=/opt/gcc/491/lib64:$LD_LIBRARY_PATH
export CFLAGS="-O2 -g -pipe -Wall -Wp,-D_FORTIFY_SOURCE=2 -fexceptions -fstack-protector --param=ssp-buffer-size=4 -m64 -march=native -fsanitize=thread -fno-omit-frame-pointer -pie -fPIC -fdiagnostics-color=auto" ; # cflags common to us, libevent2

# local path of scratch ldms files
build_subdir=LDMS_thdobj

# full path of where we want things installed
prefix=`pwd`/LDMS_thdinst
...
if test -f lib/packaging/ovis-lib-toss.spec.in; then
	...
	if test -f $expected_event2_prefix/include/event2/event.h; then
		...
		if test "$LOCALEVENT" = "1"; then
			if test -f $expected_event2_prefix/include/event2/event.h; then
				...
				mkdir -p .build-eventthd
				cp $eventname.tar.gz .build-eventthd
				(cd .build-eventthd && ../packaging/g49thd.libevent2 $expected_event2_prefix )
				if test -f $expected_event2_prefix/include/event2/event.h; then
	...
	allconfig="--prefix=$prefix --enable-rdma  --with-libevent=$expected_event2_prefix --disable-sos --disable-perfevent --disable-zap --disable-zaptest --disable-swig --enable-auth LDFLAGS=-fsanitize=thread"
	../configure $allconfig && \
	make && \
	make install && \
