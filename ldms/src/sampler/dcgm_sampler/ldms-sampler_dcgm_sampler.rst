...
Standalone mode requires the nv-hostengine daemon to be running.
...
nv-hostengine daemon. nv-hostengine must be disabled to allow Embedded
mode.
...
**config**
   ...
   embedded=<\*>
      ...
      | Any value given enables the NVidia DCGM library's embedded mode, which
        elimintates the dependence on the external nv-hostengine daemon.
