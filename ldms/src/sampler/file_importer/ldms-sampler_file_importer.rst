...
**config**\  **name**\ =<cfg_name> **plugin**\ =file_importer *<SAMPLER_BASE_OPTIONS>*  *<OPTIONS>*
...
-----------
   ...
   **sample_tag**\ *=<TAG>*
      ...
      The message *tag* to which the sampler will subscribe. Messages
      beginning the string "import" will cause the sampler to process
      all files in the configured *sample_path*. The default value is
      "file_importer_import".
   ...
   **data_xprt**\ *-<XPRT>*
      ...
      The transport type for publishing data, e.g. 'sock', 'rdma', or 'fabric'.

   **data_host**\ *-<HOSTNAME>*

      The host to which JSON message data will be published. The
      default is "localhost".

   **data_tag**\ *=<TAG>*

      The name of the LDMS message tag on which the imported file
      data will be published. The data is published with this *TAG* at
      either the sample interval or when an "import" message is
      received with the *sample_tag*.

   **data_uid**\ *=<user-id>*

      The user-id that will be included in JSON messages published to
      *data_host*.  Note that the **ldmsd** daemon must be owned by
      *root:root* to publish as anyone other than *geteuid()*

   **data_gid**\ *=<grouop-id>*

      The group-id that will included in JSON messages published to
      *data_host*.  Note that the **ldmsd** daemon must be owned by
      *root:root* to publish as anyone other than *getegid()*

   **data_perm**\ *=<octal-mask>*

      The permission bits placed in the published message data. See *open()* for detail
      on this value. The default is 0664.

   **reconnect**\ *=<SECS>*

      The number of seconds between connection retries to
      *data_host*. The default value for this is 'localhost'.
