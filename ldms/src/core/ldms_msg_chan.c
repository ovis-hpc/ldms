...
static void reconnect_cb(ovis_event_t ev)
{
	...
	default:
		ovis_log(chan_log, OVIS_LERROR,
			 "%s:%d Receiveed OVIS event on channel '%s' "
			 "in invalid state %d.\n",
			 __func__, __LINE__,
			 chan->app_name_s, chan->stats.state);
		break;
	}
...
static void connect_cb(ldms_t x, ldms_xprt_event_t e, void *cb_arg)
{
	...
	pthread_mutex_lock(&chan->lock);
	ovis_log(chan_log, OVIS_LINFO,
		 "%s:%d Channel '%s' (%s %s:%d:%s[%s])"
		 " state: %d event type: %s\n",
		 __func__, __LINE__,
		 chan->app_name_s,
		 chan->xprt_s,
		 "", 0, chan->auth_s, av_to_string(chan->auth_avl, 1),
		 chan->stats.state,
		 ldms_xprt_event_type_to_str(e->type));
	switch(e->type) {
	case LDMS_XPRT_EVENT_CONNECTED:
		ovis_log(chan_log, OVIS_LINFO,
			 "%s:%d Channel '%s' --> LDMS_MSG_CHAN_CONNECTED\n",
			 __func__, __LINE__,
			 chan->app_name_s);
	...
	case LDMS_XPRT_EVENT_DISCONNECTED:
		ovis_log(chan_log, OVIS_LINFO,
			 "%s:%d Channel '%s' --> LDMS_MSG_CHAN_DISLDMS_MSG_CHAN_CONNECTED\n",
			 __func__, __LINE__,
			 chan->app_name_s);
...
static int init_listener(ldms_msg_chan_t chan)
{
	...
	rc = getaddrinfo(chan->lcl_host_s,
			 chan->lcl_port_s, &hint, &ai);
	if (rc)
		return errno;
	...
	if (rc) {
		ovis_log(chan_log, OVIS_LERROR,
			 ...
			 rc, chan->xprt_s, chan->lcl_host_s, chan->lcl_port_s,
			 chan->auth_s);
...
ldms_msg_chan_t ldms_msg_chan_new(const char *app_name_s,
				  ...
				  const char *rem_port_s,
				  const char *lcl_host_s,
				  const char *lcl_port_s,
				  const char *auth_s,
				  av_list_t auth_avl,
				  int reconnect)
{
	...
	if (!app_name_s || !rem_host_s || !rem_port_s || !lcl_port_s)  {
		errno = EINVAL;
		return NULL;
	}
	if (!lcl_host_s)
		lcl_host_s = "localhost";
	if (!auth_s)
		auth_s = "none";
	...
	chan->rem_port_s = strdup(rem_port_s);
	if (!chan->rem_port_s)
		goto err_2;
	...
	chan->lcl_port_s = strdup(lcl_port_s);
	if (!chan->lcl_port_s)
		goto err_2;
 ...
 err_2:
	...
	free(chan->rem_port_s);
	free(chan->lcl_host_s);
	free(chan->lcl_port_s);
	free(chan->auth_s);
...
static void flush_msg_queue(ldms_msg_chan_t chan)
{
	...
	while (!TAILQ_EMPTY(&chan->msg_q)) {
		...
		if (rc) {
			ovis_log(chan_log, OVIS_LERROR,
				 "Insufficient privilege to publish message as %d:%d\n",
				 msg_entry->cred.uid, msg_entry->cred.gid);
			break;
		}
		chan->stats.pub_msg_cnt ++;
...
{
	chan->stats.max_q_depth = q_depth;
}
...
const char *ldms_msg_chan_state_str(ldms_msg_chan_state_t state)
{
	...
	if (state >= 0 && state < sizeof(state_s[0]))
