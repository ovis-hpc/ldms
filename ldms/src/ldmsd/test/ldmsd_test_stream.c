...
jbuf_t make_test_json()
{
        int i;
        jbuf_t jb;
	json_entity_t test_list;
	json_entity_t list_name;

        jb = jbuf_new();
        jb = jbuf_append_str(jb, "{");
        jb = jbuf_append_attr(jb, "schema", "\"%s\",", "stream_test");
        jb = jbuf_append_attr(jb, "timestamp", "%d,", 1559242264);
	jb = jbuf_append_attr(jb, "data", "{");
	jb = jbuf_append_attr(jb, "id", "%d,", 12345);
	jb = jbuf_append_attr(jb, "test_list", "%s", "[1,2,3]");
...
int main(int argc, char **argv)
{
	...
	if (!ldms)
		printf("Error creating ldms daemon\n");
	test_json = make_test_json();
	int rc = ldmsd_stream_publish(ldms, stream, LDMSD_STREAM_JSON, test_json->buf, test_json->cursor);
	if (rc)
		printf("Error %d uploading json data.\n", rc);
	else
		printf("The json data was successfully uploaded.\n");
	/*
	rc = ldmsd_stream_publish(ldms, stream, LDMSD_STREAM_STRING, "string_test", sizeof("string_test"));
	if (rc)
		printf("Error %d uploading string\n", rc);
	else
		printf("String data uploaded successfully\n");*/
	return 0;
}
