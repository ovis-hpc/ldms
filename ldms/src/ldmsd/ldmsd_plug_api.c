...
ldmsd_plug_handle_t ldmsd_strgp_to_handle(ldmsd_strgp_t strgp)
{
        return (ldmsd_plug_handle_t)strgp->store;
}
...
void *ldmsd_plug_api_get(ldmsd_plug_handle_t handle)
{
        ...
        switch (cfg->type) {
        case LDMSD_CFGOBJ_SAMPLER:
                return (void *)((ldmsd_cfgobj_sampler_t)cfg)->api;
        case LDMSD_CFGOBJ_STORE:
                return (void *)((ldmsd_cfgobj_store_t)cfg)->api;
        default:
                ovis_log(NULL, OVIS_LERROR,
                         "ldmsd_plug_api_get(): handle is not a plugin cfgobj\n");
                return NULL;
        }
}
