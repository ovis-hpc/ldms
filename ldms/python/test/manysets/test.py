...
# SYNOPSIS
# --------
# server-mode (listening on HOST:PORT):
#   ./test.py -x XPRT -p PORT -h HOST [-n NUM_SETS] -s
# client-mode (connecting to HOST:PORT):
#   ./test.py -x XPRT -p PORT -h HOST [-n NUM_SETS]
#
#
# DESCRIPTIONS
# ------------
# The script runs in two modes: server-mode and client-mode.
# The server-mode process creates sets (specified by NUM_SETS) and ldms_listen
# on the specified HOST/PORT/XPRT. The client process initialize LDMS memory
# according to NUM_SETS and connects to the server process (specified by
# HOST/PORT/XPRT). Then, the client performs `ldms_dir`, `ldms_lookup` and
# periodically `ldms_updates`.
...
psr.add_argument("-n", "--num-sets", default=1024, type=int,
